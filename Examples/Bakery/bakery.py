...
if __name__ == "__main__":
    ...
    def process(i):
        ...
        for j in range(N):
            stmts.append(f"pc_{i} = 1 & " + has_max_number(j) + f" & next(pc_{i}) = 2 & next(number_{i}) = number_{i} & next(tmp_{i}) = number_{j}")  # determine maximum
        stmts.append(f"pc_{i} = 2 & next(pc_{i}) = 3 & next(number_{i}) = tmp_{i} + 1 & next(tmp_{i}) = tmp_{i}")  # draw number    
        stmts.append(f"pc_{i} = 3 & " + can_enter(i) + f" & next(pc_{i}) = 4 & next(number_{i}) = number_{i} & next(tmp_{i}) = tmp_{i}")  # trying to critical
        stmts.append(f"pc_{i} = 3 & !" + can_enter(i) + f" & next(pc_{i}) = 3 & next(number_{i}) = number_{i} & next(tmp_{i}) = tmp_{i}")  # trying to critical
